...
```json
{"action":"prepare","source":"<Kakao URL 또는 slug>","layoutMode":"auto","ownerUserId":<kakao_stickers.owner_user_id>,"pack":"<kakao_stickers.default_pack_alias>","packTitle":"<kakao_stickers.default_pack_title>","packSlug":"<kakao_stickers.default_pack_slug>","emoji":"<kakao_stickers.default_emoji>","dataDir":"<kakao_stickers.data_dir>"}
```
...
- `sourceTraits`, `layout`의 판정 근거·캔버스·콘텐츠 박스·경고
...
카카오 링크를 준비할 때 미니 이모지는 slug나 제목으로 추측하지 않고 카카오 API의 `contents.isMini`를 기준으로 판별한다. 미니 이모지는 일반 이모티콘처럼 512px를 꽉 채우지 않고 **512×512 투명 캔버스에 원본 전체 프레임을 최대 250×250으로 비율 유지해 중앙 배치**한다. crop이나 프레임별 alpha bounding box 재정렬은 사용하지 않는다.

`layoutMode`의 기본값은 `auto`이며, API 신호가 없을 때는 조용히 일반 레이아웃을 적용하지 않고 중단한다. API 변경을 처리해야 할 때만 `mini` 또는 `standard` 수동 재정의를 사용하며, 충돌은 prepare 결과의 경고로 표시한다. 자세한 판정·변환 계약은 `references/mini-emoji-detection-and-conversion.md`를 따른다.
...
- `references/mini-emoji-detection-and-conversion.md`
